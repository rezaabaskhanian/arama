...
func (h Handler) SetAdminRoutes(e *echo.Echo) {
	...
	// مدیریت تمرین‌های واقعی زندگی (Commitment templates)
	adminGroup.GET("/commitment-templates", h.GetAllCommitmentTemplates, middlware.Auth(h.authSvc, h.authConfig), middlware.AdminOnly)
	adminGroup.POST("/commitment-templates", h.CreateCommitmentTemplate, middlware.Auth(h.authSvc, h.authConfig), middlware.AdminOnly)
	adminGroup.PUT("/commitment-templates/:id", h.UpdateCommitmentTemplate, middlware.Auth(h.authSvc, h.authConfig), middlware.AdminOnly)
	adminGroup.DELETE("/commitment-templates/:id", h.DeleteCommitmentTemplate, middlware.Auth(h.authSvc, h.authConfig), middlware.AdminOnly)
}
